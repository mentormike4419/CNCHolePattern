...
//int user_input() {
//    // user types number of holes (x) on terminal
//    int holes{};
//    std::cout << "1x Aluminum Hole Pattern Generator\n";
...
int main() {
    ...
    // input hole number via user terminal or Autogen (1-50)
    // request x hole value from user termimal
    // int holes{ user_input() };
    // or 
    // Autogen process creates files for 1 throug holes .tap files
    int holes{ 5 }; // max number of holes
    int Hcount{ 1 }; // loop Hcount from 1 to holes
    while (Hcount <= holes) {
        //create output filename
        std::string fname = ("1x" + std::to_string(Hcount) + "_Alum_HolePattern.tap");
